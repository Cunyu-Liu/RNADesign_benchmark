...
def evaluate(df, test_scores, K_list=(1, 3, 5), out_prefix="result"):
    ...
    te["score"] = np.asarray(test_scores, dtype=float)
        ...
        for tid, g in te.groupby("target_id"):
            ...
            if K == 1:
                per_target.append({"target_id": tid, "success_at_1": success_at_k(ranked, succ, 1),
                                   ...
                                   "regret10": normalized_regret(ranked, rel, 10),
                                   "top1": ranked[0]})
    ...
    pd.DataFrame(per_target).to_csv(f"{out_prefix}_per_target.csv", index=False)
    return agg
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="/mnt/cunyuliu/ToeholdDesignBench/processed/runner_result")
    a = ap.parse_args()
    ...
    if a.scores:
        ...
        scores = te["score"].fillna(0).values
    else:
