# P3 — v0.1 baseline results (archived; superseded by v0.2)

> **Do not cite these values.** The report predates the v0.2 CNN channel-layout,
> signed-label, five-seed, confidence-interval, and Pareto-metric corrections.
> Replacement results are generated by `src/p3_baselines.py` and
> `src/p3_robustness.py` with `_v02` output names.
